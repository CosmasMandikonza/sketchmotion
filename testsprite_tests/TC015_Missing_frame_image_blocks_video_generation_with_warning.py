"""TC015 — Frames without images: Generate Video disabled + ai-video-frames-not-ready."""
import asyncio
import re
...
async def run_test():
    async with async_playwright() as pw:
        ...
        try:
            ...
            gen = page.get_by_test_id("ai-video-generate-button").first
            await expect(gen).to_be_disabled()

            warn = page.get_by_test_id("ai-video-frames-not-ready").first
            await expect(warn).to_be_visible()
            wtext = await warn.inner_text()
            assert "polish" in wtext.lower() or "missing" in wtext.lower(), wtext
